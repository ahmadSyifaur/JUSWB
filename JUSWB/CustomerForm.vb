...
Public Class CustomerForm
    ...
    Dim IDCust, NameCust, CodeCust, AddressCust, telp_noCust, updated_byCust, updated_atCust, created_atCust As String
    ...
    Private Sub save_Record(sender As Object, e As EventArgs)
        ...
        Try
            ExecuteNonQuery("insert into " & TableName & " (customer_code,name,address,telp_no,updated_by,created_at,updated_at) values ('" & tb_customerCode.Text & "','" & tb_name.Text _
                            & "','" & tb_address.Text & "','" & tb_telpNo.Text & "','" & username & "','" & todaysdate & "','" & todaysdate & "')", SQLiteCon)
    ...
    Private Sub btn_refresh_Click(sender As Object, e As EventArgs) Handles btn_refresh.Click
        ...
        Try
            ...
            DataGridViewTable.Columns("customer_code").HeaderText = "Customer Code"
    ...
    Private Sub btn_edit_Click_1(sender As Object, e As EventArgs) Handles btn_edit.Click
        PanelInput.Show()
        tb_customerCode.Text = CodeCust
        tb_name.Text = NameCust
        tb_address.Text = AddressCust
        tb_telpNo.Text = telp_noCust
    ...
    Private Sub DataGridViewTable_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles DataGridViewTable.CellMouseClick
        ...
        With DataGridViewTable
            If e.RowIndex >= 0 Then
                i = .CurrentRow.Index
                rowSelected = True
                IDCust = .Rows(i).Cells("ID").Value.ToString
                CodeCust = .Rows(i).Cells("customer_code").Value.ToString
                NameCust = .Rows(i).Cells("name").Value.ToString
                AddressCust = .Rows(i).Cells("address").Value.ToString
                telp_noCust = .Rows(i).Cells("telp_no").Value.ToString
    ...
    Private Sub btn_delete_Click(sender As Object, e As EventArgs) Handles btn_delete.Click
        ...
        If MsgBox("Delete record " & NameCust & "?", MsgBoxStyle.Question + MsgBoxStyle.OkCancel, "Confirmation") = MsgBoxResult.Cancel Then Return
        ...
        Try
            ...
            For Each row As DataGridViewRow In DataGridViewTable.SelectedRows
                If row.Selected = True Then
                    ExecuteNonQuery("delete from " & TableName & " where ID='" & IDCust & "'", SQLiteCon)
                End If
            Next
        Catch ex As Exception
            MsgBox(ex.Message)
        ...
        Else
            ...
            If checkbox_byCode.Checked = True Then
                If tb_search.Text = Nothing Then
                    searchQuery = "select*from " & TableName & " order by Name"
                Else
                    searchQuery = "select*from " & TableName & " where customer_code like'" & tb_search.Text & "%'"
                End If
            End If
